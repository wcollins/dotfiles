...
- **`scripts/install-packages.sh`**: Debian package installer invoked by `setup.sh` on Linux. Installs apt packages (including `xclip` and `wl-clipboard` for local tmux clipboard integration) and alternative installers for tools not in apt (Starship, mise, eza, gh, 1Password CLI, lazygit, shfmt, tealdeer, yq, fastfetch, JetBrains Mono Nerd Font, and Ghostty on `desktop`). `install_neovim()` requires Neovim >= 0.11 (mason-lspconfig 2.x depends on `vim.lsp.config` / `vim.lsp.enable`); when apt's Neovim is older it falls back to the upstream AppImage at `~/.local/bin/nvim`.
- **Tmux clipboard**: `tmux/.config/tmux/copy-clipboard.sh` is the tmux-yank copy command. It uses `pbcopy`, `clip.exe`, `wl-copy`, or `xclip` only when that tool can reach a display, then always asks tmux to set the attached terminal clipboard via OSC 52. That is how a yank inside remote tmux reaches the host over SSH. `set-clipboard` is `on`, and clipboard support is advertised for `xterm*`, `tmux*`, and `screen*`. Ghostty allows clipboard writes. After updating, restow `tmux` and restart tmux (`tmux kill-server`). Check with `bash scripts/test-copy-clipboard.sh`.
...
- **`shared/functions.sh`**: Bash/Zsh-compatible helpers sourced by `.zshrc`. `hugo-lan` prompts for a host LAN IPv4 address and runs a draft-enabled Hugo preview on port 1313. Bash users can source this file directly. Check with `bash scripts/test-hugo-lan.sh`.
